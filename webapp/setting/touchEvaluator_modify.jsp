...
<s:form action="touchEvaluator!modify.action" method="post" name="myform">
	<div class="path">您现在的位置： 首页 &gt; 系统设置 &gt; 按键式评价器设置&gt; 修改触摸式评价器设置</div>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" class="public_table">
				...
				<td width="37%">
					<s:if test="#session.loginInfo.user.customsId==1">
						<s:select reg=".+" tip="请选择所属关区" id="orgId" name="touchEvaluator.orgId" list="#request.orgs" listKey="orgId" listValue="orgName" headerKey="" headerValue="--请选择--" cssClass="select_s_1"></s:select>
					</s:if>
					<s:else>
						<s:select reg=".+" tip="请选择所属关区" id="orgId" name="touchEvaluator.orgId" list="#request.orgs" listKey="orgId" listValue="orgName" headerKey="" headerValue="--请选择--" cssClass="select_s_1" disabled="true"></s:select>
				...
				<td width="37%">
					<s:hidden name="touchEvaluator.id"></s:hidden>
