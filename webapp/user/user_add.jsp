...
<s:form action="user!insert.action" method="post" name="myform" enctype="multipart/form-data">
	<div class="path">您现在的位置： 首页 &gt; 权限管理 &gt; 用户管理 &gt; 新增用户</div>
	<div class="public_div">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" class="public_table">
			 <tr>
			    ...
			    <td width="39%"><input type="text" name="user.userName" reg="^[a-z0-9A-Z]" tip="用户名只能为字母或数字或字母与数字的组合" maxlength="30" class="input_s_1"/>
                      <span class="red">*</span></td>
			    <td width="11%" class="td_w_s text_r"><span class="text_r">真实姓名：</span></td>
			    <td width="39%"><input type="text" name="user.realName" class="input_s_1" reg="^\S" maxlength="20" tip="姓名为必填项" />
                      <span class="red">*</span></td>
		  	 </tr>
		  	 <tr>
				<td class="td_w_s text_r">工号：</td>
				<td><input type="text" name="user.employeeNumber" class="input_s_1" maxlength="10" reg="^\S" tip="工号不能为空"/><span class="red">*</span></td>
				<td class="td_w_s text_r">职务：</td>
				<td><input type="text" name="user.position" class="input_s_1" maxlength="30" reg="^\S" tip="职务不能为空"/><span class="red">*</span></td>
			 </tr>
			 <tr>
	      	 	<td class="td_w_s text_r">报表数据访问权限：</td>
	      	 	<td><s:select name="user.dataAccessPerm" list="user.dataAccessPermList" listKey="key" listValue="value" value="'N'" cssClass="select_s_1"></s:select><span class="red">*</span></td>
	      	 	<td class="td_w_s text_r">触摸屏评价：</td>
	      	 	<td><s:select name="user.isEvaluated" list="user.isEvaluatedList" listKey="key" listValue="value" value="'N'" cssClass="select_s_1"></s:select><span class="red">*</span></td>
	      	 </tr>
			 <tr>
                <td class="td_w_s text_r">所属机构：</td>
			    <td>
			    	<input type="text" id="orgSel" value="" class="input_s_2" readonly="readonly" onclick="showOrgTree();" />
			    	<input type="hidden" name="user.orgId" value="" />
			    ...
			    <td>
                	<s:radio name="user.sex" list="user.sexList" listKey="key" listValue="value" value="'U'" label="性别"></s:radio>
				</td>
	      	 </tr>
			 <tr>
                <td class="td_w_s text_r">办公电话：</td>
			    <td><input type="text" name="user.telephone" reg="^((\d+[-])*\d+)*$" maxlength="30" tip="电话号码格式不正确" class="input_s_1" /></td>
			    <td class="td_w_s text_r"><span class="text_r">手机号码：</span></td>
			    <td><input type="text" name="user.mobile" class="input_s_1" maxlength="30"/></td>
	      	 </tr>
			 <tr>
			    <td class="td_w_s text_r"><span class="text_r">邮箱地址：</span></td>
			    <td><input type="text" name="user.email" class="input_s_1" reg="^\s*$|^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$" tip="邮箱地址格式不正确" maxlength="50"/></td>
			    <td class="td_w_s text_r">用户照片：</td>
			    <td>&nbsp;<input type="file" name="photo" id="photo" onchange="CheckExt(this)"/></td>
	      	 </tr>
		  </table>
	  </div>
	</div>
