...
<body class="left_body">
	...
	<div class="menu">
		<s:iterator value="menus" var="parItem">
			<h2 class="on"
				onClick="JavaScript:showHide('<s:property value="menuKey"/>');">
				<span><s:property value="menuName" />
				</span>
			</h2>
			<s:if test="%{#parItem.subMenuList.size>0}">
				<ul id='<s:property value="menuKey"/>' style="DISPLAY: none">
					<s:iterator value="#parItem.subMenuList">
						<s:if test="menuType == 3 ">

							<li><a
								href="<%=request.getContextPath()%><s:property value='menuUrl'/>"
								target="exMainTFrame"><span><s:property
											value="menuName" />
