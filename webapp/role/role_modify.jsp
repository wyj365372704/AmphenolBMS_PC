	...
	function extendsValidate(){	
	   var extendSubmit=true;
		...
		if($("[name='orgRes']:checked").length < 1){
			errorMessge=errorMessge+menu+".请为角色选择至少一项功能权限"+'\n';
			menu++;
			extendSubmit = false;
		}
		return extendSubmit;
...
<s:form action="role!modify.action" method="post" name="myform">
	<div class="path">您现在的位置： 首页 &gt; 权限管理 &gt; 角色管理 &gt; 修改角色</div>
		<div class="public_div">
			...
			<s:hidden name="role.roleId"/>
				  <tr>
					...
					<td>
						<s:if test='role.roleName=="分关管理员"||role.roleName=="系统管理员"'>
							<s:textfield name="role.roleName" cssClass ="input_s_2" disabled="true"/>
							<s:hidden name="role.roleName"></s:hidden>
						</s:if>
						<s:else>
							<s:textfield name="role.roleName" reg="^[a-zA-Z0-9\u4e00-\u9fa5]{1,25}$" maxlength="25" tip="角色名称为必填项且不能包含特殊字符" cssClass ="input_s_2"/>
						</s:else>
						<input type="hidden" name="role.oldRoleName" value="${role.roleName }" />
				    	<span class="red">*</span>
				    </td>
				  </tr>
				  <tr>
					...
					<td>
						<s:if test='orgTreeData==""'>
							<input type="text" name="" value="<s:property value='role.orgName' />" disabled class="input_s_2" /> 
							<s:hidden name="role.orgId"/>
						</s:if>
						<s:else>
							<input type="text" id="orgSel" value="${role.orgName}" class="input_s_2" readonly="readonly" onclick="showOrgTree();"/>
							<input type="hidden" id="orgId" name="role.orgId" value="${role.orgId }" />
				  ...
				  <tr>
					...
					<td colspan="3"><label>
					  <s:textarea name="role.roleDesc" cssClass ="textarea_s_1"></s:textarea>
					</label></td>
				  </tr>
			  </table>
		  </div>
