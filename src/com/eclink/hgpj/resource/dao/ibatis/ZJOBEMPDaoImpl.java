 ...
 *
 */
public class ZJOBEMPDaoImpl extends SqlMapClientDaoSupport implements ZJOBEMPDao {

	@Override
	public void insertZjobemp(ZJOBEMPVO vo) throws Exception {
		getSqlMapClientTemplate().insert("ZJOBEMP.insertZjobemp", vo);	
	}

	@Override
	public List<ZJOBEMPVO> queryByMapWithEmpName(Map map) throws Exception {
		return getSqlMapClientTemplate().queryForList("ZJOBEMP.queryByMapWithEmpName", map);		
	}
	
	public void updateZjobempJstat(ZJOBEMPVO vo) throws Exception{
		getSqlMapClientTemplate().update("ZJOBEMP.updateZjobempJstat", vo);
	}
}
