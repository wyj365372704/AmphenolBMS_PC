package com.eclink.hgpj.resource.vo;

import java.math.BigDecimal;
import java.util.List;

import com.eclink.hgpj.base.BaseVO;

/**
 * @Title: 资源值对象类  
 * @Description: 

 * @version 1.0
 * 
 */
public class MODATAVO extends BaseVO{
	
	private static final long serialVersionUID = 2317968819354022841L;
	
	private String ordno;
	
	private String citem;
	
	private String citwh;
	
	private BigDecimal qtypre;
	
	private BigDecimal qtreq;
	
	private BigDecimal isqty;
	
	private String unmsr;
	
	private String cdesc;
	
	private BigDecimal seqnm;
	
	private String fitem;
	
	private String dptno;
	
	private String blcf;
	
	private String whsub2;
	
	private String llocn2;
	
	private BigDecimal orqty;
	
	private BigDecimal qtdev;
	
	private BigDecimal turna;
	
	private BigDecimal turnn;

	private BigDecimal turnc;
	
	private BigDecimal listd;

	private String usrsq;
	
	private String lblot;
	
	private String uugam2;

	private ZITEMBXVO zitembx;
	
	public String getOrdno() {
		return ordno;
	}

	public void setOrdno(String ordno) {
		this.ordno = ordno;
	}

	public String getCitem() {
		return citem;
	}

	public String getUugam2() {
		return uugam2;
	}

	public void setUugam2(String uugam2) {
		this.uugam2 = uugam2;
	}

	public void setCitem(String citem) {
		this.citem = citem;
	}

	public String getCitwh() {
		return citwh;
	}

	public void setCitwh(String citwh) {
		this.citwh = citwh;
	}

	public BigDecimal getQtypre() {
		return qtypre;
	}

	public void setQtypre(BigDecimal qtypre) {
		this.qtypre = qtypre;
	}

	public BigDecimal getQtreq() {
		return qtreq;
	}

	public void setQtreq(BigDecimal qtreq) {
		this.qtreq = qtreq;
	}

	public BigDecimal getIsqty() {
		return isqty;
	}

	public void setIsqty(BigDecimal isqty) {
		this.isqty = isqty;
	}

	public String getUnmsr() {
		return unmsr;
	}

	public void setUnmsr(String unmsr) {
		this.unmsr = unmsr;
	}

	public String getCdesc() {
		return cdesc;
	}

	public void setCdesc(String cdesc) {
		this.cdesc = cdesc;
	}


	public BigDecimal getSeqnm() {
		return seqnm;
	}

	public void setSeqnm(BigDecimal seqnm) {
		this.seqnm = seqnm;
	}

	public String getFitem() {
		return fitem;
	}

	public void setFitem(String fitem) {
		this.fitem = fitem;
	}

	public String getDptno() {
		return dptno;
	}

	public void setDptno(String dptno) {
		this.dptno = dptno;
	}

	public String getBlcf() {
		return blcf;
	}

	public void setBlcf(String blcf) {
		this.blcf = blcf;
	}

	public String getWhsub2() {
		return whsub2;
	}

	public void setWhsub2(String whsub2) {
		this.whsub2 = whsub2;
	}

	public String getLlocn2() {
		return llocn2;
	}

	public void setLlocn2(String llocn2) {
		this.llocn2 = llocn2;
	}

	public BigDecimal getOrqty() {
		return orqty;
	}

	public void setOrqty(BigDecimal orqty) {
		this.orqty = orqty;
	}

	public BigDecimal getQtdev() {
		return qtdev;
	}

	public void setQtdev(BigDecimal qtdev) {
		this.qtdev = qtdev;
	}

	public BigDecimal getTurna() {
		return turna;
	}

	public BigDecimal getListd() {
		return listd;
	}

	public void setListd(BigDecimal listd) {
		this.listd = listd;
	}

	public void setTurna(BigDecimal turna) {
		this.turna = turna;
	}

	public BigDecimal getTurnn() {
		return turnn;
	}

	public void setTurnn(BigDecimal turnn) {
		this.turnn = turnn;
	}

	public BigDecimal getTurnc() {
		return turnc;
	}

	public void setTurnc(BigDecimal turnc) {
		this.turnc = turnc;
	}

	public String getUsrsq() {
		return usrsq;
	}

	public void setUsrsq(String usrsq) {
		this.usrsq = usrsq;
	}

	public String getLblot() {
		return lblot;
	}

	public void setLblot(String lblot) {
		this.lblot = lblot;
	}

	public ZITEMBXVO getZitembx() {
		return zitembx;
	}

	public void setZitembx(ZITEMBXVO zitembx) {
		this.zitembx = zitembx;
	}
	
	
}